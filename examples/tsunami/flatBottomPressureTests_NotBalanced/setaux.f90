...
!
    ...
    !! turn off here since need actual pressure forcing, which isn't constant now
    ext_press = .8d0 * ambient_pressure
    if (1==0 .and. pressure_forcing) then
        !!aux(pressure_index, :, :) = ambient_pressure
        aux(pressure_index, :, :) = .5d0*ambient_pressure
    endif
...
!        to reduce round off discrepancies
    ...
    do jj=1-mbc,my+mbc
        ...
        do ii=1-mbc,mx+mbc
            ...
            else if (pressure_forcing .and. coordinate_system==1) then 
               !! setting variable pressure here
               radius = dsqrt(x*x+y*y)
               aux(pressure_index,ii,jj) = ext_press * exp(-0.5d0*(radius/10000)**2)
            endif
